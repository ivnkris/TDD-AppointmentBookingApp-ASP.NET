...
namespace AppointmentBooking.Domain.Database
{
    public static class SeedData
    {
        public static async Task Initialize(IServiceProvider serviceProvider)
        {
            ...
            if (await context.Customers!.AnyAsync()) return; // DB has been seeded
            ...
            var john = await context.AddAsync(new Customer { FirstName = "John", LastName = "Undercut" });
            var ian = await context.AddAsync(new Customer { FirstName = "Ian", LastName = "TaperFade" });

            // Services
            var mensCut = await context.AddAsync(new Service { Name = "Men's Cut", AppointmentTimeSpanInMin = 30, Price = 23, IsActive = true });
            var mensClipperScissor = await context.AddAsync(new Service { Name = "Men - Clipper & Scissor Cut", AppointmentTimeSpanInMin = 30, Price = 23, IsActive = true });
            var mensBeardTrim = await context.AddAsync(new Service { Name = "Men - Bear Trim", AppointmentTimeSpanInMin = 10, Price = 10, IsActive = true });
            var mensColoring = await context.AddAsync(new Service { Name = "Men - Full Head Coloring", AppointmentTimeSpanInMin = 70, Price = 60, IsActive = true });
            var mensPerm = await context.AddAsync(new Service { Name = "Men - Perm", AppointmentTimeSpanInMin = 100, Price = 90, IsActive = true });
            var mensKeratin = await context.AddAsync(new Service { Name = "Men - Keratin Treatment", AppointmentTimeSpanInMin = 120, Price = 100, IsActive = true });
            var boysCut = await context.AddAsync(new Service { Name = "Boys - Cut", AppointmentTimeSpanInMin = 30, Price = 15, IsActive = true });
            var girlsCut = await context.AddAsync(new Service { Name = "Girls - Cut", AppointmentTimeSpanInMin = 30, Price = 17, IsActive = true });
